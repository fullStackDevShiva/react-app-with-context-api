...
const Card = ({ tripPackage }: { tripPackage: TripPackage }) => {
  ...
  return (
    <div className={`card ${selectedTheme} h-96 p-3`}>
      ...
      <div className="w-full h-1/6 items-center text-center p-2">
        <p className="text-xl font-bold -mt-2">{tripPackage.name}</p>
        <p className="font-semibold mt-1">{`${tripPackage.price}/person`}</p>
      </div>
